...
import tech.jensen.coma.api.util.CombatUtil;
import tech.jensen.coma.api.util.ItemUtil;
...
extends Module {
    ...
    private final Setting<Boolean> mendingTakeOff = this.register( new Setting <> ( "Auto Mend" , false ));
    private final Setting<Integer> crystalRange = this.register( new Setting <> ( "Crystal Range" , 10 , 0 , 20 ));
    ...
    private final Setting<Boolean> curse = this.register( new Setting <> ( "Curse Of Binding" , false ));
    private final Setting<Integer> actions = this.register( new Setting <> ( "Actions" , 3 , 1 , 12 ));
    private final Setting<Boolean> tps = this.register( new Setting <> ( "Tps Sync" , true ));
    ...
    public void onUpdate() {
        ...
        if (this.taskList.isEmpty()) {
            ...
            ItemStack chest;
            int slot4;
            if ( this.mendingTakeOff.getValue ( ) && this.mc.player.getHeldItemMainhand().getItem() == Items.EXPERIENCE_BOTTLE && this.mc.gameSettings.keyBindUseItem.isKeyDown() && (this.isSafe() || BlockUtil.isSafe( this.mc.player , 1))) {
                int bootDamage;
                ItemStack feet2;
                int leggingDamage;
                ItemStack legging2;
        ...
        if (this.timer.hasReached((int)((float) this.delay.getValue ( ) * ( this.tps.getValue ( ) ? TrollGod.INSTANCE.getTpsManager().getTpsFactor() : 1.0f)))) {
            if (!this.taskList.isEmpty()) {
    ...
    public List<Integer> findEmptySlots(boolean withXCarry) {
        ...
        if (withXCarry) {
            for (int i = 1; i < 5; ++i) {
                Slot craftingSlot = this.mc.player.inventoryContainer.inventorySlots.get(i);
    ...
    private Map<Integer, ItemStack> getInventorySlots(int currentI, int last) {
        ...
        for (int current = currentI; current <= last; ++current) {
            fullInventorySlots.put(current, this.mc.player.inventoryContainer.getInventory().get(current) );
        }
    ...
    private boolean isSafe() {
        Entity crystal = this.mc.world.loadedEntityList.stream().filter(e -> e instanceof EntityEnderCrystal).filter(e -> this.mc.player.getDistance(e) > (float) this.crystalRange.getValue ( ) ).findFirst().orElse(null);
        if (crystal == null) {
            EntityPlayer closest = CombatUtil.getTarget( this.closestEnemy.getValue ( ) );
            if (closest == null) {
                return true;
            }
            return this.mc.player.getDistanceSq( closest ) >= this.square( this.closestEnemy.getValue ( ) );
        }
        return false;
    }
    ...
    private static class Task {
        ...
        public void run() {
            if (this.slot != -1) {
                this.mc.playerController.windowClick(0, this.slot, 0, this.quickClick ? ClickType.QUICK_MOVE : ClickType.PICKUP, this.mc.player );
            }
            if (this.update) {
