...
extends Module {
    private final Setting<Integer> tickDelay = this.register( new Setting <> ( "Delay" , 1 , 0 , 5 ));
    private final Setting<Integer> threshold = this.register( new Setting <> ( "Threshold" , 30 , 1 , 63 ));
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        this.mc.playerController.windowClick(0, inventorySlot, 0, ClickType.PICKUP, this.mc.player );
        this.mc.playerController.windowClick(0, hotbarSlot, 0, ClickType.PICKUP, this.mc.player );
        this.mc.playerController.windowClick(0, inventorySlot, 0, ClickType.PICKUP, this.mc.player );
    }
    ...
    private int findCompatibleInventorySlot(ItemStack hotbarStack) {
        ...
        for (Map.Entry<Integer, ItemStack> entry : this.getInventory().entrySet()) {
            ...
            if (inventoryStack.isEmpty() || inventoryStack.getItem() == Items.AIR || !this.isCompatibleStacks(hotbarStack, inventoryStack) || smallestStackSize <= (currentStackSize = this.mc.player.inventoryContainer.getInventory().get( entry.getKey ( ) ).getCount())) continue;
