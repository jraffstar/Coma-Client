...
import tech.jensen.coma.api.mixin.accessors.IPlayerControllerMP;
import tech.jensen.coma.api.property.Setting;
import tech.jensen.coma.api.util.Timer;
import tech.jensen.coma.client.events.ClickBlockEvent;
...
extends Module {
    private final Setting<Integer> delay = this.register( new Setting <> ( "Delay" , 20 , 0 , 500 ));
    ...
    public void onUpdate() {
        if (this.renderBlock != null && this.breakTimer.hasReached( this.delay.getValue ( ) )) {
            this.mc.player.connection.sendPacket( new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, this.renderBlock, this.direction) );
    ...
    public void onDamageBlock(ClickBlockEvent event) {
        ...
        if (this.canBreak(event.getPos())) {
            if (this.lastBlock == null || event.getPos() != this.lastBlock) {
                ...
                this.mc.player.connection.sendPacket( new CPacketPlayerDigging(CPacketPlayerDigging.Action.START_DESTROY_BLOCK, event.getPos(), event.getFacing()) );
                this.packetCancel = true;
            } else {
                this.packetCancel = true;
            }
            this.mc.player.connection.sendPacket( new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, event.getPos(), event.getFacing()) );
    ...
    private boolean canBreak(BlockPos pos) {
        ...
        return block.getBlockHardness(blockState, this.mc.world , pos) != -1.0f;
    }
    ...
    public void setTarget(BlockPos pos) {
        ...
        this.packetCancel = false;
        this.mc.player.connection.sendPacket( new CPacketPlayerDigging(CPacketPlayerDigging.Action.START_DESTROY_BLOCK, pos, EnumFacing.DOWN) );
        this.packetCancel = true;
        this.mc.player.connection.sendPacket( new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, pos, EnumFacing.DOWN) );
