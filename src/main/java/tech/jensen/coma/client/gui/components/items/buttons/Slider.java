package tech.jensen.coma.client.gui.components.items.buttons;
...
import tech.jensen.coma.client.gui.TrollGui;
import tech.jensen.coma.client.gui.components.Component;
...
extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y, this.x + (float)this.width + 7.0f, this.y + (float)this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? 0x11555555 : -2007673515);
        RenderUtil.drawRect(this.x, this.y, ((Number)this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float)this.width + 7.0f) * this.partialMultiplier(), this.y + (float)this.height - 0.5f, 0x70555555);
        TrollGod.fontManager.drawString(this.getName() + " " + ChatFormatting.GRAY + (this.setting.getValue() instanceof Float ? (Number) this.setting.getValue() : (Number)((Number)this.setting.getValue()).doubleValue()), this.x + 2.0f, this.y - 1.0f - (float)TrollGui.getClickGui().getTextOffset(), -1);
    }
