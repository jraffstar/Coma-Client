package tech.jjjen.coma.api.util.font;
    ...
    public float drawString(String text, double x, double y, int color, boolean shadow) {
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            if (character == '\u00A7') {
                ...
                if (strikethrough)
                    drawLine(x, y +  (float)currentData[character].height / 2, x + currentData[character].width - 8.0D, y +  (float)currentData[character].height / 2);
                if (underline)
                    drawLine(x, y +  (float)currentData[character].height - 2.0D, x + currentData[character].width - 8.0D, y + currentData[character].height - 2.0D);
